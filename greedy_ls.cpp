...
struct Solution {
  ...
  Obj _xchg(int x, int l1, int r1, int u, int l2, int r2) {
    ...
    if (!(1 <= l1 && l1 <= r1 && r1 + 1 < A.size())) return O;
    if (!(1 <= l2 && l2 <= r2 && r2 + 1 < B.size())) return O;
  ...
  Obj _opt3(int x, int i, int j, int kk) {
    ...
    auto calc = [&](bool rb, bool rc, bool sw) -> int {
      ...
      auto add_rng = [&](int l, int rr, bool rev) {
        if (!rev) {
          for (int t = l; t <= rr; t++) tmp.push_back(r[t]);
        }
        else {
          for (int t = rr; t >= l; t--) tmp.push_back(r[t]);
        }
      };
  ...
  //         int l1 = rnd(1, (ma - 2) - lenA + 1);
  //         int r1 = l1 + lenA - 1;
  //         int l2 = rnd(1, (mb - 2) - lenB + 1);
  //         int r2 = l2 + lenB - 1;
    ...
    while (el() < tl && noimp() < sl) {
      ...
      for (int t = 0; t < tries && el() < tl; t++) {
        ...
        if (coin < 28) {
          ...
          int v = pv(uu);
          if (v == -1) continue;
          if (uu == x && (v == y || v == y + 1 || v + 1 == y)) continue;
      ...
      if (best.tp != -1 && best.v < O) {
        if (best.tp == 0) 
          oneMove(best.p[0], best.p[1], best.p[2], best.p[3]);
        else if (best.tp == 1) 
          orOpt(best.p[0], best.p[1], best.p[2], best.p[3], best.p[4]);
        else if (best.tp == 2) 
          opt2(best.p[0], best.p[1], best.p[2]);
        else if (best.tp == 3) 
          swp2(best.p[0], best.p[1], best.p[2], best.p[3]);
        else if (best.tp == 4) 
          opt2s(best.p[0], best.p[1], best.p[2], best.p[3]);
        else if (best.tp == 5) 
          xchg(best.p[0], best.p[1], best.p[2], best.p[3], best.p[4], best.p[5]);
        else if (best.tp == 6) 
          opt3(best.p[0], best.p[1], best.p[2], best.p[3]);
        else if (best.tp == 7)
          flp(best.p[0]);
